...
export class PromotionService {
  ...
  getPromotionFilter(typePromo = 'open', active = null, vigency = false, search = null, searchBy = 'NAME'): Observable<any> {
    let params = `?promotionType=` + typePromo.toLocaleUpperCase();
    params = params + '&onVigency=' + vigency;
    if (active) {
      params = params + '&status=' + active;
    }
    if (search) {
      params = params + '&searchValue=' + search;
    }
    params = params + '&searchBy=' + searchBy.toLocaleUpperCase();

    return this.http.get<Array<Promotion>>(`${environment.apiUrl}v1/promotion/filter/${params}`,
      {
        observe: 'response',
        headers: this.getHttpOptions()
      }).pipe(
